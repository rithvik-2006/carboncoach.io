...
export function LiveActivityFeed({ initialData }: { initialData: FeedEvent[] }) {
  const [feed, setFeed] = useState<FeedEvent[]>(initialData)
  const supabase = createClient()

  useEffect(() => {
    const channel = supabase.channel('global-feed-channel')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'global_activity_feed' }, (payload) => {
  ...
  return (
    <Card className="h-full border-muted/60 shadow-sm bg-card/50 backdrop-blur-sm flex flex-col">
      <CardHeader className="pb-3 flex flex-row items-center gap-2">
        ...
        <CardTitle className="text-lg">Live Sustainability Feed</CardTitle>
      </CardHeader>
      <CardContent className="flex-1 min-h-0 p-0">
        <ScrollArea className="h-[300px] w-full px-6 pb-4">
          <div className="space-y-4 pr-4">
            ...
            {feed.length === 0 && (
              <div className="text-sm text-muted-foreground text-center py-4">Waiting for community activity...</div>
            )}
          </div>
        </ScrollArea>
      </CardContent>
